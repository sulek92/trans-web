...
export function HomePageClient({
  ...
  const cmsData = (initialCmsData ?? {}) as Record<string, unknown>;
  ...
  const heroTitle = typeof cmsData.heroTitle === 'string'
    ? cmsData.heroTitle
    : 'Transport paletowy bez niespodzianek.';
  const heroSubtitle = typeof cmsData.heroSubtitle === 'string'
    ...
    <main className="pt-24 pb-16 bg-[var(--color-background)]">
      ...
      <section className="max-w-[1280px] mx-auto px-8 mb-24 animate-fade-in">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-gutter items-center">
          {/* Left Column */}
          <div className="lg:col-span-7 flex flex-col gap-lg pr-8">
            <div>
              <span className="inline-block px-3 py-1 bg-[var(--color-primary-highlight)] text-[var(--color-on-primary-fixed-variant)] font-label-sm text-[14px] font-bold rounded-full mb-6 border border-[var(--color-primary)] transition-premium hover:scale-105 cursor-default">Logistyka B2B dla profesjonalistów</span>
              <h1 className="font-display-bold text-[56px] leading-[1.1] text-[var(--color-on-background)] mb-6 font-bold tracking-tight">
              ...
              <p className="font-body-base text-[20px] text-[var(--color-on-surface-variant)] max-w-xl leading-relaxed mb-10">
                {heroSubtitle}
              </p>
      ...
      <section className="bg-white py-16 border-y border-[var(--color-divider)]">
        <div className="max-w-[1280px] mx-auto px-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-12 items-center justify-items-center opacity-40 grayscale">
            {['DHL Freight', 'FedEx Express', 'Raben', 'DSV', 'DB Schenker'].map((p, i) => (
      ...
      <section className="py-24 bg-slate-50 border-y border-[var(--color-divider)]">
        <div className="max-w-[1280px] mx-auto px-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-16 items-center">
            <div>
              <h2 className="text-4xl font-bold mb-6">Zawsze do Twojej dyspozycji</h2>
            ...
            <div className="bg-white p-8 rounded-[40px] shadow-2xl border border-slate-100 relative z-10">
              ...
              <form 
                onSubmit={(e) => {
                  ...
                  simulateSend('support', 'Twoja wiadomość została wysłana! Odprowiemy w ciągu 15 minut.', () => target.reset());
                }}
                className="flex flex-col gap-4"
              >
                <input required type="text" placeholder="Imię i nazwisko" className="p-4 bg-slate-50 rounded-xl border border-transparent focus:border-[var(--color-primary)] outline-none transition-colors" />
                <input required type="email" placeholder="E-mail firmowy" className="p-4 bg-slate-50 rounded-xl border border-transparent focus:border-[var(--color-primary)] outline-none transition-colors" />
                <textarea required placeholder="W czym możemy pomóc?" rows={4} className="p-4 bg-slate-50 rounded-xl border border-transparent focus:border-[var(--color-primary)] outline-none transition-colors resize-none"></textarea>
                <button 
                  type="submit" 
      ...
      <section className="mx-8 mb-24">
        <div className="max-w-4xl mx-auto bg-[#005258] rounded-[60px] p-20 text-center text-white relative overflow-hidden shadow-2xl">
          <h2 className="text-5xl font-bold mb-8">Zacznij wysyłać taniej już dziś</h2>
          <p className="text-xl opacity-80 mb-12 max-w-2xl mx-auto">Dołącz do 1200+ firm, które zaufały technologii PaletBroker. Twoja pierwsza przesyłka może być u odbiorcy już jutro.</p>
