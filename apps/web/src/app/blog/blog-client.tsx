...
export function BlogClient({ newsletterTitle, newsletterDesc }: { newsletterTitle: string; newsletterDesc: string }) {
  ...
  const posts = [
    { id: 1, title: 'Jak przygotować paletę do transportu międzynarodowego?', excerpt: 'Dowiedz się, jakie wymogi muszą spełniać palety EPAL i jak zabezpieczyć towar przed długą trasą.', category: 'Poradniki', date: '05 Maj 2024', image: 'inventory_2' },
    { id: 2, title: 'Dopłata paliwowa w 2024 - co musisz wiedzieć?', excerpt: 'Analiza rynku paliw i jej wpływ na stawki transportowe w branży TSL.', category: 'Rynek', date: '02 Maj 2024', image: 'oil_barrel' },
    { id: 3, title: 'Automatyzacja logistyki w Twoim e-commerce', excerpt: 'Jak integracja API z PaletBroker może skrócić czas obsługi zamówień o 40%.', category: 'Technologia', date: '28 Kwiecień 2024', image: 'robot_2' },
  ];
            ...
            <Link key={post.id} href={`/blog/${post.id}`} className="group bg-white rounded-2xl overflow-hidden border border-[var(--color-divider)] shadow-sm hover:shadow-xl transition-premium flex flex-col">
              ...
              <div className="p-8 flex-grow">
                <div className="flex items-center gap-3 mb-4">
                  <span className="text-[10px] font-bold uppercase tracking-widest text-[var(--color-primary)] bg-[var(--color-primary-highlight)] px-2 py-1 rounded">{post.category}</span>
                ...
                <p className="text-[var(--color-on-surface-variant)] text-sm leading-relaxed mb-6">{post.excerpt}</p>
