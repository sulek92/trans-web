  ...
  return (
    <div className="space-y-10 animate-fade-in relative pb-20">
      <div>
        ...
        <h1 className="text-4xl font-bold text-slate-900 mb-2 tracking-tight">Zarządzanie Cennikiem</h1>
        <p className="text-slate-500 text-lg">Stawki wyświetlane użytkownikom oraz reguły silnika wycen.</p>
      </div>
      ...
      <section className="bg-white p-8 rounded-[32px] border border-slate-100 shadow-sm space-y-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-400">To widzą użytkownicy na podstronie /cennik</p>
            </div>
          </div>
          <Link
            href="/admin/cms/pages/edit/cennik"
            className="inline-flex items-center gap-2 px-6 py-3 rounded-2xl bg-[var(--color-primary)] text-white font-bold text-sm hover:scale-105 transition-all shadow-lg shadow-[var(--color-primary)]/20"
          >
        ...
        {cmsData ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-slate-50 rounded-2xl p-6 border border-slate-100">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="font-bold text-sm uppercase tracking-widest text-slate-600">Stawki Krajowe</h3>
              </div>
              <div className="space-y-2">
                {(cmsData.domesticRates || []).map((r, i) => (
                  <div key={i} className="flex items-center justify-between p-3 bg-white rounded-xl border border-slate-100">
                    <div className="flex items-center gap-2">
                      <span className="material-symbols-outlined text-slate-400 text-sm">{r.icon || 'package'}</span>
                ...
                {(!cmsData.domesticRates || cmsData.domesticRates.length === 0) && (
                  <p className="text-sm text-slate-400 p-3">Brak stawek krajowych</p>
                )}
              </div>
            </div>
            ...
            <div className="bg-slate-50 rounded-2xl p-6 border border-slate-100">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="font-bold text-sm uppercase tracking-widest text-slate-600">Stawki Międzynarodowe</h3>
              </div>
              <div className="space-y-2">
                {(cmsData.internationalRates || []).map((r, i) => (
                  <div key={i} className="flex items-center justify-between p-3 bg-white rounded-xl border border-slate-100">
                    <div>
                      ...
                      <span className="text-xs text-slate-400 ml-2">{r.eta}</span>
                    </div>
                    <span className="font-bold text-[var(--color-primary)]">{r.price} PLN</span>
                  </div>
                ))}
                {(!cmsData.internationalRates || cmsData.internationalRates.length === 0) && (
                  <p className="text-sm text-slate-400 p-3">Brak stawek międzynarodowych</p>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="p-8 text-center border-2 border-dashed border-slate-100 rounded-2xl">
            <p className="text-slate-400 text-sm mb-3">Brak danych cennika w CMS.</p>
      ...
      <section className="space-y-6">
        <div className="flex justify-between items-end">
          <div>
            ...
            <p className="text-sm text-slate-400">Konfiguruj marże i ceny bazowe dla kalkulatora wycen.</p>
          </div>
          <button
