...
export default async function TermsPage() {
  const cms = await getCmsContent<{ sections: typeof FALLBACK }>('regulamin');
  const terms = cms?.sections ?? FALLBACK;
        ...
        <div className="space-y-10">
          {terms.map((t, i) => (
            <section key={i} className="border-b border-[var(--color-divider)] pb-8 last:border-0">
              ...
              <p className="text-[var(--color-on-surface-variant)] leading-relaxed text-sm">{t.content}</p>
            </section>
          ))}
        </div>
      </div>
