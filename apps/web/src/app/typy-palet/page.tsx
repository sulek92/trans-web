...
export default async function PalletTypesPage() {
  ...
  const types = cms?.palletTypes ?? FALLBACK.palletTypes;
  const tips = cms?.measurementTips ?? FALLBACK.measurementTips;
    ...
    <main className="pt-24 pb-24 bg-[var(--color-background)]">
      <div className="max-w-[1280px] mx-auto px-8">
        <div className="mb-16 text-center max-w-3xl mx-auto">
          ...
          <p className="text-[var(--color-on-surface-variant)] text-lg">Wybierz odpowiednią paletę dla swojego towaru. Prawidłowy dobór nośnika to klucz do bezpiecznego i ekonomicznego transportu.</p>
        </div>
              ...
              <div className="p-8 flex-grow flex flex-col">
                ...
                <div className="space-y-3 mb-6">
                  {[{l:'Wymiary:',v:t.dims},{l:'Waga własna:',v:t.weight},{l:'Nośność:',v:t.capacity}].map((row,j) => (
                    <div key={j} className="flex justify-between text-sm py-2 border-b border-[var(--color-divider)]">
                      <span className="text-[var(--color-on-surface-variant)]">{row.l}</span>
                      <span className="font-bold text-[var(--color-on-background)]">{row.v}</span>
                    </div>
                  ))}
                </div>
                <p className="text-[var(--color-on-surface-variant)] text-sm leading-relaxed mb-8">{t.desc}</p>
                <Link href={`/?palletType=${t.name.includes('Euro') ? 'euro' : t.name.includes('Przemysłowa') ? 'industrial' : 'half'}`} className="mt-auto w-full py-4 rounded-xl border border-[var(--color-primary)] text-[var(--color-primary)] font-bold hover:bg-[var(--color-primary)] hover:text-white transition-premium text-center block">
