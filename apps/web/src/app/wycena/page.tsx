...
import { CustomQuoteForm } from '@/components/calculator/custom-quote-form';
  ...
  React.useEffect(() => {
    const fetchQuotes = async () => {
      ...
      try {
        ...
        // Only set mock data if we don't have results yet
        if (!results) {
          setResults({
            results: [
              {
    ...
    <main className="pb-16 max-w-[1280px] mx-auto px-4 sm:px-8 min-h-screen bg-[var(--color-background)] transition-colors duration-500">
      ...
      {error && carriers.length === 0 && (
        <motion.div
          ...
          className="mb-8 rounded-3xl border border-[var(--color-error)] bg-[var(--color-error)]/5 px-6 py-5 text-sm text-[var(--color-error)] flex flex-col sm:flex-row items-center justify-between gap-4"
        >
          <div className="flex items-center gap-3">
            <span className="material-symbols-outlined text-[var(--color-error)]">cloud_off</span>
            <div className="flex flex-col">
              <span className="font-bold text-base">Błąd połączenia z serwerem wycen</span>
              <span className="opacity-80 font-medium">Nie mogliśmy pobrać aktualnych stawek. Spróbuj ponownie za chwilę.</span>
            </div>
          </div>
          <button 
            onClick={() => window.location.reload()}
            className="flex items-center gap-2 px-5 py-2 bg-[var(--color-error)] text-white rounded-xl font-bold hover:scale-105 transition-all"
          >
            Odśwież
      ...
      <motion.section
        ...
        className="rounded-[40px] bg-[var(--color-surface-primary)] border border-[var(--color-divider)] shadow-[var(--shadow-premium)] overflow-hidden mb-12"
      >
        ...
        >
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-[var(--color-text-muted)] mb-6">{results.nonStandardReason}</p>
              ...
              <CustomQuoteForm 
                ...
                weight={chargeableWeight}
                dimensions={{ length, width, height }}
                route={`${senderCode} (${senderCountry}) -> ${recipientCode} (${recipientCountry})`}
              />
            </div>
          </div>
        </motion.div>
...
{t.quote.results.offers}
      ...
      {carriers.length === 0 && !isLoading && !results?.isNonStandard && (
        <motion.div
          ...
          className="mb-14"
        >
          <div className="rounded-[40px] border-2 border-dashed border-[var(--color-divider)] p-12 text-center bg-[var(--color-surface-container)]/30 backdrop-blur-sm overflow-hidden relative">
            {/* Background pattern */}
            ...
            <div className="relative z-10">
              <div className="w-24 h-24 rounded-3xl bg-[var(--color-surface-container)] flex items-center justify-center mx-auto mb-8 shadow-[var(--shadow-premium)] border border-[var(--color-divider)]">
                <span className="material-symbols-outlined text-5xl text-[var(--color-text-faint)] animate-pulse">search_off</span>
              </div>
              <h3 className="font-bold text-3xl text-[var(--color-on-background)] mb-4 tracking-tight">Nie znaleźliśmy ofert dla podanych parametrów</h3>
              <p className="text-[var(--color-text-muted)] max-w-lg mx-auto mb-10 text-lg font-medium leading-relaxed">
                Nasi przewoźnicy nie mogą obsłużyć przesyłki o tych wymiarach lub wadze w podanej relacji. Spróbuj zmienić parametry lub skontaktuj się z nami bezpośrednio.
              </p>
              
              {/* Suggestion cards */}
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 max-w-3xl mx-auto mb-12">
                <div className="p-5 rounded-2xl bg-white/40 border border-white/60 dark:bg-slate-900/40 dark:border-slate-800 flex flex-col items-center text-center group hover:bg-white transition-colors">
                  <span className="material-symbols-outlined text-[var(--color-primary)] mb-3 group-hover:scale-110 transition-transform">scale</span>
                  <div className="text-xs font-bold uppercase tracking-widest text-[var(--color-text-faint)] mb-1">Waga</div>
                  <div className="text-sm font-bold">Zmniejsz wagę poniżej 1000kg</div>
                </div>
                <div className="p-5 rounded-2xl bg-white/40 border border-white/60 dark:bg-slate-900/40 dark:border-slate-800 flex flex-col items-center text-center group hover:bg-white transition-colors">
                  <span className="material-symbols-outlined text-[var(--color-primary)] mb-3 group-hover:scale-110 transition-transform">straighten</span>
                  <div className="text-xs font-bold uppercase tracking-widest text-[var(--color-text-faint)] mb-1">Wymiary</div>
                  <div className="text-sm font-bold">Standard to 120x80x180cm</div>
                </div>
                <div className="p-5 rounded-2xl bg-white/40 border border-white/60 dark:bg-slate-900/40 dark:border-slate-800 flex flex-col items-center text-center group hover:bg-white transition-colors">
                  <span className="material-symbols-outlined text-[var(--color-primary)] mb-3 group-hover:scale-110 transition-transform">support_agent</span>
                  <div className="text-xs font-bold uppercase tracking-widest text-[var(--color-text-faint)] mb-1">Pomoc</div>
                  <div className="text-sm font-bold">Wycena indywidualna</div>
                </div>
              </div>
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
                <Link
                  href={editUrl}
                  className="w-full sm:w-auto inline-flex items-center justify-center gap-3 px-10 py-4 bg-[var(--color-primary)] text-white font-bold rounded-2xl hover:bg-[var(--color-surface-tint)] transition-all shadow-xl shadow-[var(--color-primary)]/25 hover:scale-[1.02] active:scale-[0.98]"
                >
                ...
                <Link
                  href="/kontakt"
                  className="w-full sm:w-auto inline-flex items-center justify-center gap-3 px-10 py-4 bg-white border border-[var(--color-divider)] text-[var(--color-on-background)] font-bold rounded-2xl hover:bg-[var(--color-surface-container)] transition-all hover:scale-[1.02] active:scale-[0.98]"
                >
...
function CarrierCard({ carrier, onOrder, resolvedQuoteId }: { carrier: QuoteCarrier; onOrder: () => void; resolvedQuoteId: string }) {
  const { t } = useTranslation();
  ...
  return (
    <motion.div
      ...
      className="relative rounded-[32px] border border-[var(--color-divider)] bg-[var(--color-surface-primary)] overflow-hidden transition-all duration-300 group hover:border-[var(--color-primary)]/30 hover:shadow-xl shadow-sm"
    >
        ...
        <div className="border-t border-[var(--color-divider)] pt-6">
          <div className="flex items-end justify-between mb-4">
            <div>
              ...
              <div className="text-[11px] text-[var(--color-text-faint)] font-bold uppercase tracking-widest mt-1">
                PLN brutto
              </div>
            </div>
            ...
            <button 
              ...
              className="flex items-center gap-1.5 text-[10px] font-bold text-[var(--color-primary)] bg-[var(--color-primary)]/10 px-3 py-1.5 rounded-full hover:bg-[var(--color-primary)]/20 transition-colors"
            >
              Szczegóły
          ...
          {/* Expanded Details (Surcharges) */}
          <motion.div
            initial={false}
            animate={{ height: isExpanded ? 'auto' : 0, opacity: isExpanded ? 1 : 0 }}
            className="overflow-hidden"
          >
            <div className="py-4 space-y-2 border-t border-dashed border-[var(--color-divider)] mt-2">
              {carrier.surchargeBreakdown && Object.entries(carrier.surchargeBreakdown).map(([name, value]) => (
                <div key={name} className="flex justify-between text-xs font-medium">
                  <span className="text-[var(--color-text-muted)]">{name}</span>
                  <span className="text-[var(--color-on-background)] tabular-nums">+{value.toFixed(2)} PLN</span>
                </div>
              ))}
              <div className="flex justify-between text-xs font-bold pt-2 border-t border-[var(--color-divider)] mt-2">
                <span className="text-[var(--color-on-background)]">Razem netto</span>
                <span className="text-[var(--color-on-background)] tabular-nums">{carrier.priceNetto?.toFixed(2)} PLN</span>
              </div>
            </div>
          </motion.div>
          ...
          <Link
            ...
            onClick={onOrder}
            className="w-full mt-4 inline-flex items-center justify-center gap-2 px-6 py-4 rounded-2xl font-bold text-base transition-all bg-[var(--color-surface-container)] hover:bg-[var(--color-primary)] hover:text-white text-[var(--color-on-background)] active:scale-[0.97] shadow-sm"
          >
            {t.pricing.result.orderNow}
